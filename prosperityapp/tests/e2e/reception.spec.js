...
test.describe('Recepción de Pedidos E2E Flow', () => {
  ...
  test('T14.1 - debería renderizar el stepper con 6 pasos', async ({ page }) => {
    ...
    for (const label of stepLabels) {
      // Usamos el texto exacto sin flag i y lo forzamos a ser visible
  ...
  test('T14.2 - el Paso 1 debería mostrar el importador de facturas', async ({ page }) => {
    ...
    // Y también el botón de entrada manual con texto exacto
    const manualBtn = page.locator('button').filter({ hasText: /^✏️ Ingresar datos manualmente sin factura$/ }).first();
  ...
  test('T14.3 - el botón "Ingresar manualmente" avanza al paso 2', async ({ page }) => {
    ...
    const step2Title = page.locator('h2').filter({ hasText: /^Datos del proveedor$/ }).first();
  ...
  test('T14.4 - el botón "Volver" debe navegar hacia atrás', async ({ page }) => {
    // Primero avanzar al paso 2 para que aparezca el botón Volver o que sea interactuable
    const manualBtn = page.locator('button').filter({ hasText: /^✏️ Ingresar datos manualmente sin factura$/ }).first();
    await expect(manualBtn).toBeVisible();
    await manualBtn.click();

    // Verificar que aparece y se puede clickear
  ...
  test('T14.5 - los pasos completados del stepper deben marcarse con ✓', async ({ page }) => {
    ...
    // El primer paso (índice 0) debería mostrar ✓ como completado y tener la clase correspondiente
...
test.describe('Pedidos Pendientes E2E Flow', () => {
  ...
  test('T14.6 - debería cargar la lista de pedidos', async ({ page }) => {
    ...
    const title = page.locator('h1, h2').filter({ hasText: /Pedidos y Proveedores|Orders/i }).first();

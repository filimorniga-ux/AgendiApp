  ...
  const handleSave = async (e) => {
    ...
    try {
      const dataToSave = {
        ...
        durationMin: parseInt(formData.duration) || 0,
      };

      if (isEditMode) {
        const { error } = await sbUpdate('services', serviceToEdit.id, dataToSave);
        if (error) throw error;
      } else {
        const { error } = await sbCreate('services', dataToSave, businessId);
        if (error) throw error;
      }
      ...
      <div className="bg-bg-secondary rounded-lg shadow-xl border border-border-main w-full max-w-lg modal-content flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-grow space-y-4">
          <div>
            ...
            <input type="text" name="name" id="name" placeholder={t('modals.service.placeholders.name')} value={formData.name || ''} onChange={handleInputChange} className="w-full bg-bg-tertiary border border-border-main rounded p-2 mt-1 text-text-main placeholder-text-muted/50" required />
          </div>

          <div>
            <label htmlFor="category" className="block text-sm font-medium text-text-muted">{t('modals.service.category')}</label>
            <input type="text" name="category" id="category" placeholder={t('modals.service.placeholders.category')} value={formData.category || ''} onChange={handleInputChange} className="w-full bg-bg-tertiary border border-border-main rounded p-2 mt-1 text-text-main placeholder-text-muted/50" list="category-suggestions" />
            <datalist id="category-suggestions">
              <option value="Cortes y Lavado" /><option value="Coloración" /><option value="Tratamientos" /><option value="Manicura y Pedicura" />
            </datalist>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <CurrencyInput name="price" id="price" placeholder={t('modals.service.placeholders.price')} value={formData.price} onChange={handleInputChange} className="w-full bg-bg-tertiary border border-border-main rounded p-2 mt-1 text-text-main" required />
            </div>
            <div>
              <label htmlFor="duration" className="block text-sm font-medium text-text-muted">{t('modals.service.duration')}</label>
              <input type="number" name="duration" id="duration" placeholder={t('modals.service.placeholders.duration')} value={formData.duration || ''} onChange={handleInputChange} className="w-full bg-bg-tertiary border border-border-main rounded p-2 mt-1 text-text-main" />
            </div>
          </div>
        </form>
...
// ===== FIN: src/components/modals/ServiceModal.jsx =====
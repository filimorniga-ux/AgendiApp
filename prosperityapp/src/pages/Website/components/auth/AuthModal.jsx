...
export const AuthModal = ({ isDarkMode, onClose, title, children }) => (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-md flex items-end sm:items-center justify-center sm:p-4 modal-backdrop" onClick={onClose}>
        <div className={`${isDarkMode ? 'bg-slate-950/70 border border-white/10 backdrop-blur-2xl text-white shadow-[0_8px_32px_rgba(0,0,0,0.3)]' : 'bg-white/80 border border-slate-200 backdrop-blur-2xl shadow-xl'} w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 md:p-8 relative modal-content transition-all duration-300 max-h-[90dvh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
